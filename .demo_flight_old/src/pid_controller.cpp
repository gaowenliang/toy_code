...
void ekf_callback(const demo_flight::ekf_data& ekf)
{
    ...
    ctrl_data[0] = ctrl_x -> update(ekf.position_error.x, dt);
    ctrl_data[1] = ctrl_y -> update(ekf.position_error.y, dt);
    ctrl_data[2] = ctrl_z -> update(-ekf.position_error.z, dt);
    ctrl_data[3] = ctrl_yaw -> update(-ekf.yaw_error, dt);//there is diffrent yaw in body frame and vicon frame
