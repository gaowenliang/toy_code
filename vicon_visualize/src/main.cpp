...
ros::Publisher stick_path_pub;
ros::Publisher stick_plot_pub;

nav_msgs::Path path_msg;
std_msgs::Float32 plot_msg;

nav_msgs::Path stick_path_msg;
std_msgs::Float32 stick_plot_msg;
...
void stick_pose_feedback(const geometry_msgs::TransformStamped::ConstPtr &cmd)
{
    geometry_msgs::PoseStamped pose_to_add;
    pose_to_add.pose.position.x = cmd->transform.translation.x;
    pose_to_add.pose.position.y = cmd->transform.translation.y;
    pose_to_add.pose.position.z = cmd->transform.translation.z;
    stick_plot_msg.data = cmd->transform.translation.z;
    stick_path_msg.poses.push_back(pose_to_add);
    //ROS_INFO("%4.3f %4.3f %4.3f \n", pose_to_add.pose.position.x, pose_to_add.pose.position.y, pose_to_add.pose.position.z);
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber stick_sub   = n.subscribe("/vicon/wand/wand",  100, stick_pose_feedback);
  path_pub = n.advertise<nav_msgs::Path>("vis_path", 100);
  stick_path_pub = n.advertise<nav_msgs::Path>("stick_vis_path", 100);
  plot_pub = n.advertise<std_msgs::Float32>("vis_pos_x", 100);
  stick_plot_pub = n.advertise<std_msgs::Float32>("stick_vis_pos_x", 100);
  ...
  while(n.ok())
  {
    ...
    stick_path_pub.publish(stick_path_msg);
    stick_plot_pub.publish(stick_plot_msg);
    r.sleep();
  }
